...
def get_long_live_bearer_token(dyn_client, namespace="default", sub_string="default-token"):
    """
    ...
    try:
        ...
        logger.exception("Specified substring %s doesn't exist in namespace %s", sub_string, namespace)
    except ProtocolError as e:
        # See https://github.com/kubernetes-client/python/issues/1225
        logger.info("Skip %s... because kubelet disconnect client after default 10m...", e)  
...
def get_site_response(site_url, bearer_token):
    ...
    try:
        ...
        logger.exception("Failed to connect %s due to refused connection or unsuccessful status code %s",
                         site_url, e)
...
def modify_file_content(file_name):
    ...
    with fileinput.FileInput(file_name, inplace=True, backup='.bak') as file:
        for line in file:
            print(line.replace('SENSOR_TEMPERATURE_ENABLED: "false"', 'SENSOR_TEMPERATURE_ENABLED: "true"'), end='')
